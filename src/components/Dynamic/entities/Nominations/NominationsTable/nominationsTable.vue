<template>
    <Table class="nominations-table table-wrap" v-magnifier="{ scale, size }" ref="wrap">
        <template #head>
        ...
        <template #body>
            <Row v-for="(value, index) in valueList" :key="index" class="nominations-table__row">
                <template #content>
                    ...
                    <Cell class="nominations-table__cell-paw"  v-if="value.paw === 'true'" :tag="'td'">
                        <template #content><span class="paw-btn"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const wrap = ref(null)
const isOn = ref(props.magnifer)
const scale = ref(2.0)
const size  = ref(80)
let ctrl = null

function getHostEl(r) {
  // ref на DOM → сам DOM; ref на компонент → его $el; ref-массив → первый элемент
  const v = r?.value
...
onMounted(async () => {
  ...
  // 1) сразу попробуем взять контроллер (если директива уже смонтирована)
  ctrl = el.__magnifier__ || null

  // 2) и на всякий случай подпишемся на событие готовности
  el.addEventListener('magnifier-ready', (e) => {
    ctrl = e.detail
    // пример: сразу включить лупу
    ctrl.enable()
  }, { once: true })
})
onBeforeUnmount(() => { ctrl = null })

watch(() => props.magnifer, () => {
    console.log('magnifer', props.magnifer)
    isOn.value = props.magnifer
})
